## 1) Different gene annotations (NCBI vs Ensemble/Gencode vs HGNC) & completeness of cross-reference
...
## Explore
tail -n+2 hgnc_complete_set.txt | awk -F"\t" '{print $6}' | sort | uniq -c ##  42181 Approved, 1761 Entry Withdrawn
...
tail -n+2 Homo_sapiens.gene_info | awk 'BEGIN{FS="\t";}{print $13}' | sort | uniq -c
...
###########
## 2. discrepancies between databases
...
###############
3. Ambiguity of human gene symbols within the same database due to symbol duplication:
